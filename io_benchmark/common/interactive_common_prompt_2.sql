prompt If you make use of ASMM or AMM (*TARGET parameters) you should be able to set up a separate small buffer cache
prompt on the fiy via ALTER SYSTEM SCOPE = MEMORY commands.
...
prompt
prompt Alternatively you can also use a non-default block size tablespace and assign a very small buffer cache for that
prompt non-default block size.
...
prompt In that case you can try to make use of the "Parallel Execution" option when running Enterprise Edition
prompt to speed up the object creation part.
