--------------------------------------------------------------------------------
--
-- File name:   max_write_iops_benchmark_harness.sql
--
...
--              If you want a I/O pattern mixture of random read and write I/O then the buffer cache used should be much smaller than this (see below) otherwise you'll see write activity mostly (mainly log writer, DBWR => random write IOPS, undo, archiver)
--              Please note that when it comes to conventional DML things are more complicated than with pure read IOPS measurement, since there is always the log writer and undo involved
--              So the measured write IOPS rate is - depending on your storage / disk configuration - possibly a mixture of log writer (plus archiver if running ARCHIVELOG, plus possible Standby activity) and DBWR activity
--              Furthermore depending on your Undo configuration running this benchmark might increase the size of your Undo tablespace if configured with Autoextend ON
--
...
define slave_name = "max_write_iops_benchmark"
...
and (
metric_name in ('Physical Reads Per Sec', 'Physical Read IO Requests Per Sec', 'I/O Requests per Second', 'Physical Read Total IO Requests Per Sec')
or metric_name in ('Physical Writes Per Sec', 'Physical Write IO Requests Per Sec', 'Physical Write Total IO Requests Per Sec')
)
),
step1 as
(
...
step2 as
(
select * from base where metric_name = 'Physical Write IO Requests Per Sec'
union all
select * from base where metric_name = 'Physical Writes Per Sec'
and not exists (select null from base where metric_name = 'Physical Write IO Requests Per Sec')
union all
select * from base where metric_name = 'Physical Write Total IO Requests Per Sec'
and not exists (select null from base where metric_name = 'Physical Write IO Requests Per Sec')
and not exists (select null from base where metric_name = 'Physical Writes Per Sec')
),
step3 as
(
select * from step1
