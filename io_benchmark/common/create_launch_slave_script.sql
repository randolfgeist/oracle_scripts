...
select
        ...
        then 'host start sqlplus $username/$pwd.$connect_string @$slave_name._slave ' || rownum || ' $testtype $wait_time'
        else 'host nohup sqlplus $username/$pwd.$connect_string @$slave_name._slave ' || rownum || ' $testtype $wait_time 2>&1 &'
        end
        as task
from
        dual
